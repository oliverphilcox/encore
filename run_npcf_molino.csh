...
foreach simroot ( Om_m Om_p h_m h_p s8_m s8_p alpha_m alpha_p \
          ...
          sigma_logM_m sigma_logM_p fiducial_ZA Mnu_p Mnu_pp Mnu_ppp ns_m ns_p Ob2_m Ob2_p )
  ...
  while ($nn <= $mock_max)
    ...
    echo
    echo "Starting simulation $nn with type $simroot"
    echo
    ...
    /bin/ls -l >> $errlog
    /bin/cp $errlog .
    tar cfz $root.tgz $root.*.out $root.*pcf.txt $root.*2pcf_mult?.txt $errfile run_npcf_molino.csh
    popd > /dev/null
