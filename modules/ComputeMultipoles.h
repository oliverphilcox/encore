...
*/
	...
	if (_gpumode > 0 && (dcnt > nmax-nthresh || icnt > maxp-pthresh || ne == (grid->nf)-1)) {
	  printf("Running add_pairs_and_multipoles kernel with %ld threads after cell %d\n", dcnt, ne);
          if(thread==0) accmult.Start();
  #ifdef PERIODIC
          gpu_add_pairs_and_multipoles_periodic(msave, posx, posy, posz, weights, csave, pnum, spnum, snp, sc, x0i, x2i, delta_x, delta_y, delta_z, (int)dcnt, NBIN, ORDER, NMULT, rmin, rmax, lastcnt, grid->cellsize);
  #else
          gpu_add_pairs_and_multipoles(msave, posx, posy, posz, weights, csave, pnum, spnum, snp, sc, x0i, x2i, (int)dcnt, NBIN, ORDER, NMULT, rmin, rmax, lastcnt);
  #endif
          if(thread==0) accmult.Stop();
