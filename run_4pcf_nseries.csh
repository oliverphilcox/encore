...
#SBATCH -n 16 # cpus
#SBATCH -N 1 # tasks
#SBATCH -t 0-05:59:59 # time
#SBATCH --mem-per-cpu=1GB
#SBATCH -o /home/ophilcox/out/nseries4pcf_run.%a.out         # File to which STDOUT will be written (make sure the directory exists!)
#SBATCH -e /home/ophilcox/out/nseries4pcf_run.%a.err         # File to which STDERR will be written
...
#SBATCH --array=1-84 # array of inputs
...
# File directories
set root = nseries${sim_no} # root for data filenames
set ranroot = nseries # root for random filenames
set in = /projects/QUIJOTE/Oliver/nseries_mocks/ # input directory (see above for required contents)
set out = /projects/QUIJOTE/Oliver/npcf/nseries # output file directory
set tmp = /scratch/gpfs/ophilcox/nseries_${sim_no} # temporary directory for intermediate file storage for this run (ideally somewhere with fast I/O)
...
cp run_npcf_patchy_disc.csh $tmp
...
# Define output coupling file (to avoid recomputation of disconnected pieces)
...
gunzip -c $in/$ranroot.ran.0.gz > $tmp/$root.ran.0

echo "Starting R^N" >> $errlog
date >> $errlog
($command -in $tmp/$root.ran.0 -outstr $root.r -invert > $tmpout/$root.r.out) >>& $errlog
...
echo Combining NPCF files together and performing edge-corrections
python python/combine_files.py $tmpout/$root >>& $errlog $OMP_NUM_THREADS

# Repeat for the disconnected piece
echo Combining disconnected 4PCF pieces together and performing edge corrections
...
/bin/ls -l >> $errlog
/bin/cp $errlog .
tar cfz $root.tgz $root.*.out $root.*pcf.txt $root.*2pcf_mult?.txt $errfile run_4pcf_nseries.csh
popd > /dev/null
