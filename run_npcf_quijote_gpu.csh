...
#SBATCH -o /home/ophilcox/out/quijote_real_4pcf_gpu_run.%a.out         # File to which STDOUT will be written (make sure the directory exists!)
#SBATCH -e /home/ophilcox/out/quijote_real_4pcf_gpu_run.%a.err         # File to which STDERR will be written
...
# File directories
set in = /projects/QUIJOTE/Oliver/npcf/quijote_halos # input directory (see above for required contents)
set tmp = /scratch/gpfs/ophilcox/quijote_$SLURM_ARRAY_TASK_ID
set ranroot = halos_z05
set out = /projects/QUIJOTE/Oliver/npcf/quijote_halos_gpu
...
set command = "$code -rmax $rmax -rmin $rmin -ngrid $ngrid  -scale $scale -boxsize $boxsize -gpu 1"
...
cp run_npcf_quijote_real.csh $tmp
...
while ($nn <= $mock_max)
  ...
  echo
  echo "Starting simulation $nn"
  echo

  set sim_no = $nn
  set root = halos_hr_z05_real$nn
  ...
  /bin/ls -l >> $errlog
  /bin/cp $errlog .
  tar cfz $root.tgz $root.*.out $root.*pcf.txt $root.*2pcf_mult?.txt $errfile run_npcf_quijote_real.csh
  popd > /dev/null
