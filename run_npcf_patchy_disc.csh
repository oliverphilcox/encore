...
#SBATCH -n 16 # cpus
#SBATCH -N 1 # tasks
#SBATCH -t 0-01:59:59 # time
#SBATCH --mem-per-cpu=1GB
#SBATCH -o /home/ophilcox/out/patchy4pcfNd_run.%a.out         # File to which STDOUT will be written (make sure the directory exists!)
#SBATCH -e /home/ophilcox/out/patchy4pcfNd_run.%a.err         # File to which STDERR will be written
...
#SBATCH --array=1-100 # array of inputs
...
set out = /projects/QUIJOTE/Oliver/npcf/boss_4pcfN_production_disc # output file directory
set tmp = /scratch/gpfs/ophilcox/npcf4N_${sim_no} # temporary directory for intermediate file storage for this run (ideally somewhere with fast I/O)
...
set RRR_coupling = $out/$root.RRR_coupling.npy
...
python combine_disconnected_files.py $tmpout/$root 4 $RRR_coupling >>& $errlog
...
/bin/ls -l >> $errlog
/bin/cp $errlog .
tar cfz $root.tgz $root.*.out $root.*pcf.txt $root.*2pcf_mult?.txt $errfile run_npcf_patchy_disc.csh
popd > /dev/null
