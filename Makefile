...
# Key parameters to change are listed as compilation flags in `MODES'. Here we can specify which N-point functions are computed, whether to use periodic boundary conditions, whether to include odd-parity NPCFs and various hardware settings.
...
MODES = -DFOURPCF -DDISCONNECTED -DOPENMP -DALLPARITY
...
# Add the -DALLPARITY flag to compute both odd- and even-parity multiplets, rather than just even.
...
NVCCFLAGS = -ccbin g++ -m64  $(MODES) -gencode arch=compute_60,code=sm_60 -gencode arch=compute_61,code=sm_61 -gencode arch=compute_70,code=sm_70 -gencode arch=compute_75,code=sm_75 -gencode arch=compute_80,code=sm_80 -gencode arch=compute_86,code=sm_86 -gencode arch=compute_86,code=compute_86
...
#CXX = icpc
#CXXFLAGS= -O2 -xCORE-AVX512 -qopt-zmm-usage=high -qopenmp $(MODES)
#CXXFLAGS= -O2 -xCORE-AVX512 -qopt-zmm-usage=high -qopenmp ${CUFLAGS} $(MODES)
