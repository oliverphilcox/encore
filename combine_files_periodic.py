...
if len(sys.argv)!=6:
    raise Exception("Need to specify the input files, number of galaxies, boxsize, R_min and R_max!")
else:
    inputs = str(sys.argv[1])
    N_gal = int(sys.argv[2])
    boxsize = np.float64(sys.argv[3])
    R_min = np.float64(sys.argv[4])
    R_max = np.float64(sys.argv[5])
...
for N in Ns:
    ...
    if N==2:
        counts_tmp = np.loadtxt(tmp_file,skiprows=5) # skipping rows with radial bins
    else:
        counts_tmp = np.loadtxt(tmp_file,skiprows=5+N) # skipping rows with radial bins

    # Extract ells and radial bins
    if N==2:
        bin1 = np.loadtxt(tmp_file,skiprows=4,max_rows=1)
    elif N==3:
        ell_1 = np.asarray(counts_tmp[:,0],dtype=int)
        max_ell = np.max(ell_1)
        bin1,bin2 = np.loadtxt(tmp_file,skiprows=6,max_rows=2)
    elif N==4:
        ell_1,ell_2,ell_3 = np.asarray(counts_tmp[:,:3],dtype=int).T
        max_ell = np.max(ell_1)
        bin1,bin2,bin3 = np.loadtxt(tmp_file,skiprows=6,max_rows=3)
    elif N==5:
        ell_1,ell_2,ell_12,ell_3,ell_4 = np.asarray(counts_tmp[:,:5],dtype=int).T
        max_ell = np.max(ell_1)
        bin1,bin2,bin3,bin4 = np.loadtxt(tmp_file,skiprows=6,max_rows=4)
    elif N==6:
        ell_1,ell_2,ell_12,ell_3,ell_123,ell_4,ell_5 = np.asarray(counts_tmp[:,:7],dtype=int).T
        max_ell = np.max(ell_1)
        bin1,bin2,bin3,bin4,bin5 = np.loadtxt(tmp_file,skiprows=6,max_rows=5)
    else:
    ...
    if N==2:
        n_r = 1.*len(bin1)
        R_analyt = N_gal*nV(bin1,n_r)
    elif N==3:
        n_r = len(np.unique(np.concatenate([bin1,bin2])))
        R_analyt = N_gal*nV(bin1,n_r)*nV(bin2,n_r)/(4.*np.pi)
    elif N==4:
        n_r = len(np.unique(np.concatenate([bin1,bin2,bin3])))
        R_analyt = N_gal*nV(bin1,n_r)*nV(bin2,n_r)*nV(bin3,n_r)/(4.*np.pi)**1.5
    elif N==5:
        n_r = len(np.unique(np.concatenate([bin1,bin2,bin3,bin4])))
        R_analyt = N_gal*nV(bin1,n_r)*nV(bin2,n_r)*nV(bin3,n_r)*nV(bin4,n_r)/(4.*np.pi)**2.
    elif N==6:
        n_r = len(np.unique(np.concatenate([bin1,bin2,bin3,bin4,bin5])))
        R_analyt = N_gal*nV(bin1,n_r)*nV(bin2,n_r)*nV(bin3,n_r)*nV(bin4,n_r)*nV(bin5,n_r)/(4.*np.pi)**2.5
